...
@section('title', 'Kuesioner PIECES')
@section('page-heading', 'Kuesioner PIECES')
...
@section('content')
    <div class="container-fluid px-0">
        ...
        <form action="{{ route('kuesioner-pieces.store') }}" method="POST">
            @csrf
            ...
            @empty
                <div class="card border-0 shadow-sm" style="border-radius: 6px;">
                    <div class="card-body text-center py-5 text-muted">
                        ...
                        <p class="text-muted small mb-0">Belum ada butir pertanyaan untuk instrumen kuesioner PIECES saat
                            ini.</p>
                    </div>
                </div>
            @endforelse
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (sudahMengisi) {
                // JIKA SUDAH MENGISI: Tampilkan durasi statis dari database
                let totalSeconds = parseInt(document.getElementById('durasi_detik').value) || 0;
                formatDanTampilkanWaktu(totalSeconds);
            } else {
                // JIKA BELUM MENGISI: Timer berjalan real-time dari 0
                let totalSeconds = 0;
